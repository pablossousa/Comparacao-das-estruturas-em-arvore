...
void calcular500000entradas()
{
	...
	cout << "\n---500000 ENTRADAS---" << endl;
	cout << "\nTempos de Inserção nas estruturas: " << endl;

	chrono::steady_clock::time_point comecoBin = chrono::steady_clock::now();

	ifstream arquivo500000um(PATH "entrada500000.txt");

	if (arquivo500000um.is_open()) {
		while (getline(arquivo500000um, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	arquivo500000um.close();

	chrono::steady_clock::time_point fimBin = chrono::steady_clock::now();
	printf("\nÁrvore Binária: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimBin - comecoBin).count());

	chrono::steady_clock::time_point comecoAVL = chrono::steady_clock::now();

	ifstream arquivo500000dois(PATH "entrada500000.txt");

	if (arquivo500000dois.is_open()) {
		while (getline(arquivo500000dois, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	arquivo500000dois.close();

	chrono::steady_clock::time_point fimAVL = chrono::steady_clock::now();
	printf("Árvore AVL: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimAVL - comecoAVL).count());

	chrono::steady_clock::time_point comecoRB = chrono::steady_clock::now();

	ifstream arquivo500000tres(PATH "entrada500000.txt");

	if (arquivo500000tres.is_open()) {
		while (getline(arquivo500000tres, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	arquivo500000tres.close();

	chrono::steady_clock::time_point fimRB = chrono::steady_clock::now();
	printf("Árvore RB: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimRB - comecoRB).count());

	chrono::steady_clock::time_point comecoVetor = chrono::steady_clock::now();

	ifstream arquivo500000seis(PATH "entrada500000.txt");

	if (arquivo500000seis.is_open()) {
		while (getline(arquivo500000seis, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			vetor.push_back(aux);
	...
	arquivo500000seis.close();

	chrono::steady_clock::time_point fimVetor = chrono::steady_clock::now();
	printf("Vector: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimVetor - comecoVetor).count());

	chrono::steady_clock::time_point comecoMapa = chrono::steady_clock::now();

	ifstream arquivo500000quatro(PATH "entrada500000.txt");

	if (arquivo500000quatro.is_open()) {
		while (getline(arquivo500000quatro, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			mapa.insert({aux, 0});
	...
	arquivo500000quatro.close();

	chrono::steady_clock::time_point fimMapa = chrono::steady_clock::now();
	printf("Map: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimMapa - comecoMapa).count());



	chrono::steady_clock::time_point comecoMapaNO = chrono::steady_clock::now();

	ifstream arquivo500000cinco(PATH "entrada500000.txt");

	if (arquivo500000cinco.is_open()) {
		while (getline(arquivo500000cinco, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			mapaNO.insert({aux, 0});
	...
	arquivo500000cinco.close();

	chrono::steady_clock::time_point fimMapaNO = chrono::steady_clock::now();
	printf("Unordered Map: %ld ms\n", chrono::duration_cast<chrono::milliseconds>(fimMapaNO - comecoMapaNO).count());

	// ifstream arquivo500000cinco1(PATH "entrada500000.txt");

	// unordered_map <double, int>::iterator k;

	// if (arquivo500000cinco1.is_open()) {
	// 	while (getline(arquivo500000cinco1, linha)) {
	// 		aux = stof(linha);
	// 		cout << aux << endl;
	// 	}
	// }
