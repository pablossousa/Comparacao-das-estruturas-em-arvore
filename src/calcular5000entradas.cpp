...
void calcular5000entradas()
{
	...
	ifstream arquivo5000um(PATH "entrada5000.txt");

	if (arquivo5000um.is_open()) {
		while (getline(arquivo5000um, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	ifstream arquivo5000dois(PATH "entrada5000.txt");

	if (arquivo5000dois.is_open()) {
		while (getline(arquivo5000dois, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	ifstream arquivo5000tres(PATH "entrada5000.txt");

	if (arquivo5000tres.is_open()) {
		while (getline(arquivo5000tres, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
	...
	ifstream arquivo5000seis(PATH "entrada5000.txt");

	if (arquivo5000seis.is_open()) {
		while (getline(arquivo5000seis, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			vetor.push_back(aux);
	...
	ifstream arquivo5000quatro(PATH "entrada5000.txt");

	if (arquivo5000quatro.is_open()) {
		while (getline(arquivo5000quatro, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			mapa.insert({aux, 0});
	...
	ifstream arquivo5000cinco(PATH "entrada5000.txt");

	if (arquivo5000cinco.is_open()) {
		while (getline(arquivo5000cinco, linha)) {
			aux = stof(linha);
			r.key = aux;
			r.value = 1;
			mapaNO.insert({aux, 0});
	...
	// ifstream arquivo5000cinco1(PATH "entrada5000.txt");

	// unordered_map <double, int>::iterator k;

	// if (arquivo5000cinco1.is_open()) {
	// 	while (getline(arquivo5000cinco1, linha)) {
	// 		aux = stof(linha);
	// 		cout << aux << endl;
	// 	}
	// }
